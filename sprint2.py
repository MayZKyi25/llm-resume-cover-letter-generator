                ...
                for line_number, line in enumerate(file, start=1):
                    ...
                    try:
                        ...
                        if isinstance(job, dict):
                            processed_job = process_single_job(
                                job, line_number, json_file
                            )
                            if processed_job:
                                job_entries.append(processed_job)
                        else:
                            print(
                                f"Skipping invalid entry at line {line_number} in {json_file}"
                            )

                    except json.JSONDecodeError as e:
                        print(
                            f"Skipping malformed JSON at line {line_number} in {json_file}: {e}"
                        )
...
def insert_jobs(json_file, db_name="job_postings_test.db"):
    ...
    )  # Debugging: Indicate which file is being processed
    ...
    for attempt in range(retries):
        try:
            ...
            for index, job_data in enumerate(job_entries, start=1):
                ...
                if not job_id:
                    print(
                        f"Skipping job at index {index} in {json_file} due to missing ID: {job_data}"
                    )
                    continue

                print(
                    f"[{json_file}] Inserting job {index} - ID: {job_id}"
                )  # Debugging
            ...
            conn.close()
            print(
                f"[{json_file}] Successfully inserted {len(job_entries)} jobs.\n"
            ...
            if "database is locked" in str(e) and attempt < retries - 1:
                print(
                    f"[{json_file}] Database locked, retrying ({attempt + 1}/{retries})..."
                )
                time.sleep(1)
            else:
